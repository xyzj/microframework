...
func (m *Etcdv3Client) Register(opt *OptEtcd) error {
	m.svrName = opt.Name
	m.etcdKey = opt.Key
	// m.etcdKey = fmt.Sprintf("/%s/%s_%s", m.etcdRoot, opt.Key, gopsu.GetUUID1())
	if opt.Host == "" {
...
func (m *Etcdv3Client) WriteMore(keys []string, eis []*EtcdInfo) {
	...
	for k, key := range keys {
		...
		_, err = m.etcdClient.Put(ctx, key, detail, opt)
		if err != nil {
			m.logger.Error(fmt.Sprintf("write to %s failed: %s >%s", m.etcdAddr, key, err.Error()))
			continue
		}
	}
	cancel()
